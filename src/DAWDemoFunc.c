...
char RawFileName[MAX_BOARD][MAX_FILENAME_LEN];
const char* GetLastOpenedFileName(int boardIndex) {
    return RawFileName[boardIndex];
}
...
ERROR_CODES ParseConfigFile(FILE *f_ini, DAWConfig_t *ConfigVar) {
	...
	while (fgets(strline, 1000, f_ini) != NULL) { // get a line
		...
		// // Enable ACQ_TIME procedure (YES/NO)
		// if (!strcmp(strcpy(str1, str), "ACQ_TIME_ENABLED")) {
		// 	if ((str = strtok(NULL, " \r\t\n")) == NULL) { printf("No argument for %s. The command will be ignored\n", str1); continue; }
		// 	if (strcmp(str, "YES") == 0) ConfigVar->ACQ_TIME_ENABLED = 1;
		// 	else if (strcmp(str, "NO") == 0) ConfigVar->ACQ_TIME_ENABLED = 0;
		// 	else printf("%s: invalid option\n", str);
		// 	if ((str = strtok(NULL, " \r\t\n")) != NULL) printf("WARNING: too many arguments in %s. the first exceeding argument is %s\n", str1, str);
		// 	continue;
		// }
		...
		if (!strcmp(strcpy(str1, str), "ACQ_TIME")){
    		if ((str = strtok(NULL, " \r\t\n")) == NULL) { 
        		printf("No argument for %s. The command will be ignored\n", str1); 
        		continue; 
    		}
    		ConfigVar->ACQ_TIME = (int)strtol(str, NULL, 10);
    		// ConfigVar->AcqTimeEnabled = (ConfigVar->AcqTime > 0);
			printf("ACQ_TIME = %d\n", ConfigVar->ACQ_TIME);
    		if ((str = strtok(NULL, " \r\t\n")) != NULL) 
        		printf("WARNING: too many arguments in %s. the first exceeding argument is %s\n", str1, str);
    		continue;
		}
...
ERROR_CODES OpenRawFile(FILE **outfile, int BoardIndex, int FileIndex, char *path, char *fname) {
	...
	sprintf(filename, "%s%s_raw_b%d_file%d.bin", path, fname, BoardIndex, FileIndex);
	
	if ((*outfile = fopen(filename, "w")) == NULL) {
		printf("output file %s could not be created.\n", filename);
		return_code = ERR_OUTFILE_OPEN;
	}else{
		//save rawfile for external quote
		strncpy(RawFileName[BoardIndex], filename, sizeof(RawFileName[BoardIndex]));
		RawFileName[BoardIndex][sizeof(RawFileName[BoardIndex]) -1] = '\0';
	}
...
int ProgramDigitizers(int *handle, DAWConfig_t *ConfigVar) {
	...
	for (board = 0; board < ConfigVar->Nhandle; board++) {
		...
		if (ConfigVar->SyncEnable) {
			...
			ret |= CAEN_DGTZ_ReadRegister(handle[board], 0x8100, &regval);
			if (ConfigVar->BoardConfigVar[board]->StartMode == CAEN_DGTZ_SW_CONTROLLED){
				ret |= CAEN_DGTZ_WriteRegister(handle[board], 0x8100, regval | 0x00000100);
			}
			else{
				ret |= CAEN_DGTZ_WriteRegister(handle[board], 0x8100, regval | 0x00000104);
			}
		}
...
void CheckKeyboardCommands(int *handle, DAWConfig_t  *ConfigVar)
{
	...
	case 's':
		if (ConfigVar->AcqRun == 0) {
			...
			ConfigVar->AcqRun = 1;
			// if(ConfigVar->ACQ_TIME_ENABLED){
			printf("Acquisition will run for %d seconds\n", ConfigVar->ACQ_TIME);
			// }
		}
		else {
			for (i = 0; i < ConfigVar->Nhandle; i++){
